...
async function loadDataForDay(date) {
    ...
    try {
        const response = await fetch('/VDT-60/data', {
            method: 'POST',
            headers: {
...
function updateTablebody(data) {
    ...
    data.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <td>${item.cena_VDT60_min !== null && item.cena_VDT60_min !== undefined ? item.cena_VDT60_min.toFixed(2) : 'N/A'}</td>
            <td>${item.cena_VDT60_max !== null && item.cena_VDT60_max !== undefined ? item.cena_VDT60_max.toFixed(2) : 'N/A'}</td>
            <td>${item.cena_VDT60_avg !== null && item.cena_VDT60_avg !== undefined ? item.cena_VDT60_avg.toFixed(2) : 'N/A'}</td>
        `;
...
function updateCharts(data) {   
    ...
    if (window.myChart2) {
        ...
        window.myChart2.data.datasets[0].data = data.map(item => item.objem_VDT_nakup ?? null);
        window.myChart2.data.datasets[1].data = data.map(item => item.objem_VDT_predaj * -1 ?? null);
